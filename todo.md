...
* `_final_` - should protect from inheritance from `_final_ == True` classes! (Error text like: "<classname> is not an acceptable base type"). This behaviour was removed ascidentaly, we should restore it.
* ~~SegmentedView, Slider and other scrollable views can steal scroll and other mouse/touch events of parrent `_ScrollView`~~ (fixed: `_pytoui_is_scroll_container=True` on `_ScrollView` + `pytoui_scroll_hit_test` prefers scroll containers over inline widgets → wheel goes to ScrollView; `_PREFERRED_AXIS='x'` on Slider/SegmentedControl + `_AXIS_THRESHOLD=4px` disambiguation in `_touch_move`/`_mouse_dragged` → horizontal drag → control, vertical → ScrollView)
