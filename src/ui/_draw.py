...
def _arc_points(
    ...
    """Sample points along an arc (view-relative coords)."""
    if clockwise:
        sweep = end - start
        if sweep < 0:
            sweep += 2 * math.pi
        ...
        if sweep > 0:
            sweep -= 2 * math.pi
    steps = max(4, int(abs(sweep) * max(1.0, r) / 2.0))
    return [
        (
...
# Op-codes for FillPath / StrokePath byte buffer (must match decode_path in lib.rs)
_OP_MOVE: bytes = b"\x00"
_OP_LINE: bytes = b"\x01"
_OP_CUBIC: bytes = b"\x02"
_OP_QUAD: bytes = b"\x03"
_OP_CLOSE: bytes = b"\x04"

_ELLIPSE_KAPPA: float = 0.5522847498  # cubic bezier approximation factor for circles


def _encode_path_segments(segments: list) -> bytes:
    """Encode a Python path segment list into a compact byte buffer for FillPath/StrokePath.

    Coordinates are passed as view-local (no origin offset); Rust applies the
    full CTM (which includes origin translation) via self.ctm on the FrameBuffer.
    Arcs are approximated with line segments; rect/oval/rrect expand to bezier paths.
    """
    parts: list[bytes] = []
    K = _ELLIPSE_KAPPA
    ...
    for seg in segments:
        ...
        if op == "move":
            _, x, y = seg
            parts.append(_OP_MOVE + struct.pack("<ff", x, y))

        elif op == "line":
            _, x, y = seg
            parts.append(_OP_LINE + struct.pack("<ff", x, y))

        elif op == "curve":
            _, cp1x, cp1y, cp2x, cp2y, x, y = seg
            parts.append(
                _OP_CUBIC + struct.pack("<ffffff", cp1x, cp1y, cp2x, cp2y, x, y)
            )

        elif op == "qcurve":
            _, cpx, cpy, x, y = seg
            parts.append(_OP_QUAD + struct.pack("<ffff", cpx, cpy, x, y))

        elif op == "close":
            parts.append(_OP_CLOSE)

        elif op == "arc":
            _, cx, cy, r, start, end, clockwise = seg
            pts = _arc_points(cx, cy, r, start, end, clockwise)
            if pts:
                parts.append(_OP_MOVE + struct.pack("<ff", pts[0][0], pts[0][1]))
                for pt in pts[1:]:
                    parts.append(_OP_LINE + struct.pack("<ff", pt[0], pt[1]))

        elif op == "rect":
            _, x, y, w, h = seg
            parts.append(_OP_MOVE + struct.pack("<ff", x, y))
            parts.append(_OP_LINE + struct.pack("<ff", x + w, y))
            parts.append(_OP_LINE + struct.pack("<ff", x + w, y + h))
            parts.append(_OP_LINE + struct.pack("<ff", x, y + h))
            parts.append(_OP_CLOSE)
            ...
            rx, ry = w / 2.0, h / 2.0
            kx, ky = K * rx, K * ry
            parts.append(_OP_MOVE + struct.pack("<ff", cx_, cy_ - ry))
            parts.append(
                _OP_CUBIC
                + struct.pack(
                    "<ffffff", cx_ + kx, cy_ - ry, cx_ + rx, cy_ - ky, cx_ + rx, cy_
                )
            )
            parts.append(
                _OP_CUBIC
                + struct.pack(
                    "<ffffff", cx_ + rx, cy_ + ky, cx_ + kx, cy_ + ry, cx_, cy_ + ry
                )
            )
            parts.append(
                _OP_CUBIC
            ...
            r = min(r, w / 2.0, h / 2.0)
            kr = K * r
            parts.append(_OP_MOVE + struct.pack("<ff", x + r, y))
            parts.append(_OP_LINE + struct.pack("<ff", x + w - r, y))
            parts.append(
                _OP_CUBIC
                + struct.pack(
                    "<ffffff", x + w - r + kr, y, x + w, y + r - kr, x + w, y + r
                )
            )
            parts.append(_OP_LINE + struct.pack("<ff", x + w, y + h - r))
            parts.append(
                _OP_CUBIC
                + struct.pack(
                    "<ffffff",
            ...
            parts.append(_OP_LINE + struct.pack("<ff", x + r, y + h))
            parts.append(
                _OP_CUBIC
                + struct.pack(
                    "<ffffff", x + r - kr, y + h, x, y + h - r + kr, x, y + h - r
                )
            )
            parts.append(_OP_LINE + struct.pack("<ff", x, y + r))
            parts.append(
                _OP_CUBIC
                + struct.pack("<ffffff", x, y + r - kr, x + r - kr, y, x + r, y)
            )
            parts.append(_OP_CLOSE)
