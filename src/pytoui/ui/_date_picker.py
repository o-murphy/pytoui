...
if TYPE_CHECKING:
    from pytoui.ui._types import MouseWheel, _Action, _DatePickerMode
...
class _DateState:
    ...
    Receivers implement on_display_changed() and/or _on_selection_changed().
    ...
    def _notify_selection(self, sender: object) -> None:
        for r in self._receivers:
            if r is not sender and hasattr(r, "_on_selection_changed"):
...
class _DatePicker(View):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._date_btn = Button(
            title=_fmt_date(self._date.date),
            action=self._date_action,
            **btn_kw,
        )

        self._time_btn = Button(
            title=_fmt_time(self._date.date),
    ...
    def date(self, value: datetime | None):
        ...
        self._date.date = v
        self._date.set_display_date(v.year, v.month, sender=None)
        self._date.set_display_time(v.hour, v.minute, sender=None)
    ...
    @mode.setter
    def mode(self, value: _DatePickerMode) -> None:
        if value == self._mode:
            return
        self._mode = value
        self._close_popup()
    ...
    def _date_action(self, sender: Button):
        ...
        self._close_popup()
        if should_open:
            self._popup = _LiquidDatePicker(self._date)
            self._push_popup()
    ...
    def _time_action(self, sender: Button):
        ...
        self._close_popup()
        if should_open:
            self._popup = _LiquidTimePicker(self._date)
            self._push_popup()
