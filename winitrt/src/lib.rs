...
type EventCb  = extern "C" fn(i32, f64, f64);
...
static GLOBAL_PROXY: OnceLock<Arc<Mutex<EventLoopProxy<AppEvent>>>> = OnceLock::new();

fn close_window(windows: &mut HashMap<WindowId, WinState>, window_id: WindowId) {
    if let Some(st) = windows.remove(&window_id) {
        // sync_channel(1) — does not block the event loop thread
        st.done_tx.send(()).ok();
    }
}

fn get_or_start_loop() -> Arc<Mutex<EventLoopProxy<AppEvent>>> {
    GLOBAL_PROXY
        .get_or_init(|| {
            let (proxy_tx, proxy_rx) = mpsc::sync_channel::<Arc<Mutex<EventLoopProxy<AppEvent>>>>(1);

            std::thread::Builder::new()
                .name("winit-event-loop".into())
                .spawn(move || {
                    // Allow EventLoop on any thread (non-main-thread).
                    // Platform-specific extensions are imported locally via cfg,
                    // and called via UFCS to avoid trait name conflicts.
                    ...
                    // macOS: EventLoop requires the main thread — currently not supported
                    // in a background thread; on macOS winit_run must be called from main.

                    let event_loop = el_builder.build().expect("Failed to create EventLoop");

                    // Pass proxy to the main thread
                    proxy_tx
                        .send(Arc::new(Mutex::new(event_loop.create_proxy())))
                        .unwrap();
                    ...
                    let _ = event_loop.run(move |event, elwt: &EventLoopWindowTarget<AppEvent>| {
                        elwt.set_control_flow(if windows.is_empty() {
                            // No windows — wait for the next UserEvent without busy-looping
                            ControlFlow::Wait
                        } else {
                            ControlFlow::Poll
                        });
                        ...
                        match event {
                            ...
                            Event::WindowEvent { window_id, event } => {
                                match event {
                                    // Close window (X button or Escape)
                                    ...
                                    WindowEvent::RedrawRequested => {
                                        // Call Python render; it returns != 0 if
                                        // view.close() was called (view._presented = False)
                                        let should_close =
                                            if let Some(st) = windows.get_mut(&window_id) {
                                                ...
                                                if w > 0 && h > 0 {
                                                    ...
                                                    if signal == 0 {
                                                        // Copy pixels to softbuffer
                                                        if let Ok(mut buf) =
                                                            st.surface.buffer_mut()
                                                        {
                                                            let n = (w * h) as usize;
                                                            // osdbuf writes [R,G,B,A] (LE 0xAABBGGRR)
                                                            // softbuffer expects 0x00RRGGBB → swap R↔B
                                                            for i in 0..n {
                                                                let px =
                                                                    unsafe { *st.pixel_ptr.add(i) };
                                                                let r = (px >>  0) & 0xFF;
                                                                let g = (px >>  8) & 0xFF;
                                                                let b = (px >> 16) & 0xFF;
                                                                buf[i] = (r << 16) | (g << 8) | b;
                                    ...
                                    WindowEvent::CursorMoved { position, .. } => {
                                        if let Some(st) = windows.get(&window_id) {
                                            (st.event_cb)(2, position.x, position.y);
                                        }
                                    }
...
pub extern "C" fn winit_run(
    ...
    let proxy_arc = get_or_start_loop();
    // sync_channel(1) — send does not block the event loop thread
...
pub extern "C" fn winit_screen_size(w_out: *mut u32, h_out: *mut u32) {
    ...
    {
        let proxy = proxy_arc.lock().unwrap();
        proxy.send_event(AppEvent::GetScreenSize { tx }).ok();
    }
