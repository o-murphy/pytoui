...
import pyparsing as pp  # type: ignore[pyparsing,import-not-found]
...
_retained_globals = []
_cached_classes = {}  # type: ignore[var-annotated]
_cached_instances = weakref.WeakValueDictionary()  # type: ignore[var-annotated]
_tracefunc = None

c = cdll.LoadLibrary(None)  # type: ignore[arg-type]
...
_cached_parse_types_results = {}  # type: ignore[var-annotated]

def parse_types(type_encoding):
    """Take an Objective-C type encoding string
    and convert it to a tuple of (restype, argtypes) appropriate for objc_msgSend()"""
...
def get_possible_method_names(name, args, kwargs):
    # Return a list of possible ObjC method names for a given combination of arguments.
    # Basically all permutations of keyword args...
    if "_" in name:
    ...
    if len(args) == 0 and len(kwargs) == 0:
        ...
        for perm in permutations:
            ...
            if len(args) == 0:
                name2 = (
                    name
                    + "With%s_" % (_first_letter_cap(perm[0]),)
                    + "_".join(perm[1:])
                )
                if len(perm) > 1:
                    name2 += "_"
...
def resolve_cls_method(cls, name, args, kwargs):
    # Return (method_name, ordered_kwargs), or raise an AttributeError
    # if either no method could be found, or if the mapping was ambiguous.
...
def resolve_instance_method(obj, name, args, kwargs):
    ...
    for method_name, kwarg_order in possible_names:
        try:
            ...
            # meth =
            ObjCInstanceMethod(obj, method_name, allow_property=possibly_property)
            valid_names.append((method_name, kwarg_order))
        except AttributeError:
            pass
    if len(valid_names) == 1:
...
class ObjCClass(object):
    """Wrapper for a pointer to an Objective-C class;
    acts as a proxy for calling Objective-C class methods.
    Method calls are converted to Objective-C messages on-the-fly
    -- this is done by replacing underscores in the method name with colons
    in the selector name, and using the selector and arguments
    for a call to the low-level objc_msgSend function in the Objective-C runtime.
    For example,
    calling `NSDictionary.dictionaryWithObject_forKey_(obj, key)`
    (Python) is translated to `[NSDictionary dictionaryWithObject:obj forKey:key]`
    (Objective-C).
    If a method call returns an Objective-C object,
    it is wrapped in an ObjCInstance, so calls can be chained
    (ObjCInstance uses an equivalent proxy mechanism)."""
    ...
    def get_names(cls, prefix=None):
        ...
        filtered_list = (
            class_names
            if prefix is None
            else filter_list(lambda x: x.startswith(prefix), class_names)
        )
...
class ObjCIterator(object):
    """Wrapper for an NSEnumerator object
    -- this is used for supporting `for ... in`
    iteration for Objective-C collection types (NSArray, NSDictionary, NSSet)."""
...
class ObjCInstance(object):
    """Wrapper for a pointer to an Objective-C instance;
    acts as a proxy for sending messages to the object.
    Method calls are converted to Objective-C messages on-the-fly
    -- this is done by replacing underscores in the method name with colons
    in the selector name, and using the selector and arguments
    for a call to the low-level objc_msgSend function in the Objective-C runtime.
    For example, calling `obj.setFoo_withBar_(foo, bar)` (Python) is translated
    to `[obj setFoo:foo withBar:bar]` (Objective-C).
    If a method call returns an Objective-C object,
    it is also wrapped in an ObjCInstance, so calls can be chained."""

    def __new__(cls, ptr):
        # If there is already an instance that wraps this pointer,
        # return the same object...
        # This makes it a little easier to put auxiliary data into the instance
        # (e.g. to use in an ObjC callback)
        # Note however that a new instance may be created for the same
        # underlying ObjC object if the last instance gets garbage-collected.
        ...
        if ptr and b"NSAutoreleasePool" not in class_getName(object_getClass(ptr)):
            # Retain the ObjC object, so it doesn't
            # get freed while a pointer to it exists:
    ...
    def __len__(self):
        if any(self.isKindOfClass_(c) for c in (NSArray, NSDictionary, NSSet)):
            return self.count()
        raise TypeError(
            "object of type '%s' has no len()" % (self._get_objc_classname(),)
        )
    ...
    def __getitem__(self, key):
        if self.isKindOfClass_(NSArray):
            ...
            ns_key = ns(key)
            # NOTE: Unlike Python dicts,
            # NSDictionary returns nil (None) for keys that don't exist.
            return self.objectForKey_(ns_key)
        raise TypeError(
            "%s does not support __getitem__" % (self._get_objc_classname(),)
        )
    ...
    def __delitem__(self, key):
        if self.isKindOfClass_(NSMutableArray):
            ...
            raise TypeError(
                "%s does not support __delitem__" % (self._get_objc_classname(),)
            )
    ...
    def __setitem__(self, key, value):
        if self.isKindOfClass_(NSMutableArray):
            ...
            raise TypeError(
                "%s does not support __setitem__" % (self._get_objc_classname(),)
            )
...
def _get_possible_selector_names(method_name):
    # Generate all possible selector names from a Python method name.
    # For most methods, this isn't necessary,
    # and the selector is generated simply by replacing underscores with colons,
    # but in case the selector also
    # contains underscores, the mapping is ambiguous,
    # so all permutations of colons and underscores need to be checked.
...
class ObjCClassMethodProxy(object):
    """A proxy for an ObjCClassMethod that is resolved to an actual method
    when calling it.
    A proxy may represent different methods, depending on the keyword arguments
    that are passed (and used to construct an ObjC selector from the call)."""
    def __init__(self, cls, name):
        self.cls = weakref.ref(cls)
        self.name = name
        # Determining the method for a given combination of args/kwargs
        # can be expensive, so it's cached by 'num_args/sorted_kwarg_keys'.
        # The cache contains a pair of (ObjCClassMethod, ordered_kwarg_keys).
        self.method_cache = {}
...
class ObjCClassMethod(object):
    """Wrapper for an Objective-C class method.
    ObjCClass generates these objects automatically when accessing an attribute,
    you typically don't need use this class directly."""
    def __init__(self, cls, method_name):
        ...
        if not method:
            ...
            # For selectors that contain underscores,
            # the mapping from Python method name to selector name is ambiguous.
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            if len(args) != len(argtypes) - 2:
                raise TypeError(
                    "expected %i arguments, got %i" % (len(argtypes) - 2, len(args))
                )
...
class ObjCInstanceMethodProxy(object):
    """A proxy for an ObjCInstanceMethod that is resolved to an actual method
    when calling it.
    A proxy may represent different methods, depending on the keyword arguments
    that are passed (and used to construct an ObjC selector from the call)."""
...
class ObjCInstanceMethod(object):
    """Wrapper for an Objective-C instance method.
    ObjCInstance generates these objects automatically when accessing an attribute,
    ...
    def __init__(self, obj, method_name, allow_property=True):
        ...
        if (
            ...
            and not method
            and (
                (method_name.startswith("set") and self.sel_name.endswith(":"))
                or self.sel_name.find(":") == -1
            )
        ):
            ...
            if prop:
                ...
                if method_name.startswith("set"):
                    # NOTE: The offsets/sizes are obviously incorrect...
    ...
    def __call__(self, *args, **kwargs):
        ...
        if not LP64 and restype and issubclass(restype, Structure):
            ...
            # NOTE: In order to be a little more thread-safe,
            # we need a "private" handle for objc_msgSend(...).
            # Otherwise, a different thread could modify
            # argtypes/restype before the call is made...
...
def load_framework(name):
    return NSBundle.bundleWithPath_(
        "/System/Library/Frameworks/%s.framework" % (name,)
    ).load()

# These are hard-coded for __dir__ (listing all NSObject methods
# dynamically would add tons of cruft from categories)
# NOTE: This only includes *commonly-used* methods
# (a lot of this is pretty low-level runtime stuff that isn't needed
# very often.
# It can still be listed when calling dir() on an actual instance of NSObject)
...
class ObjCBlock(object):
    ...
    def from_param(cls, param):
        if isinstance(param, ObjCBlock) or param is None:
            ...
            # Put a reference to the block into the function,
            # so it doesn't get deallocated prematurely...
            param._block = block
            return block
...
type_encodings["@?"] = ObjCBlock  # type: ignore[assignment]

def ns(py_obj):
    """Convert common Python objects to their ObjC equivalents, i.e. str => NSString,
    int/float => NSNumber, list => NSMutableArray, dict => NSMutableDictionary,
    bytearray => NSData, set => NSMutableSet.
    Nested structures (list/dict/set) are supported.
    If an object is already an instance of ObjCInstance, it is left untouched."""
...
OMMainThreadDispatcher_name = (
    b"OMMainThreadDispatcher_3" if PY3 else "OMMainThreadDispatcher"
)
try:
...
def _add_method(
    method, class_ptr, superclass, basename, protocols, is_classmethod=False
):
    """
    ...
    if hasattr(method, "encoding"):
        ...
        if not type_encoding:
            # Fall back to "action" type encoding as the default, i.e.
            # void return type, all arguments are objects...
            num_args = len(re.findall(":", sel_name))
            type_encoding = "v%i@0:8%s" % (
                sizeof(c_void_p) * (num_args + 2),
                "".join(
                    "@%i" % ((i + 2) * sizeof(c_void_p),) for i in xrange(num_args)
                ),
            )
    ...
    # derived from the selector matches the actual function:
    argspec = inspect.getargspec(method)
    if len(argspec.args) != len(argtypes):
        raise ValueError(
            "%s has %i arguments (expected %i)"
...
def create_objc_class(
    name, superclass=NSObject, methods=[], classmethods=[], protocols=[], debug=True
):
    """Create and return a new Objective-C class"""
    basename = name
    if debug:
        # When debug is True (the default) and a class
        # with the given name already exists in the runtime,
        # append an incrementing numeric suffix,
        # until a class name is found that doesn't exist yet.
        # While this does leak some memory,
        # it makes development much easier. Note however that the returned
        # class may not have the name you passed
        # in (use the return value directly instead of relying on the name)
        suffix = 1
        while True:
            try:
        ...
        # When debug is False, assume that any class is created only once,
        # and return an existing class
        # with the given name. Note that this ignores all other parameters.
...
def _objc_exception_handler(_exc):
    ...
    with open(os.path.expanduser("~/Documents/_objc_exception.txt"), "w") as f:
        ...
        f.write(
            "The app was terminated due to an Objective-C exception. "
